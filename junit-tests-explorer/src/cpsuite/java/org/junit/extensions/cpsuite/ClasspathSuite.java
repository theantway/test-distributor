...
public class ClasspathSuite extends Suite {
	 ...
	 * If none is specified only JUnit4-style TEST_CLASSES will be run. 
	 */
	@Retention(RetentionPolicy.RUNTIME)
	@Target(ElementType.TYPE)
	...
	/**
	 * The <code>BaseTypeFilter</code> annotation filters all test classes
	 * to be run by one or several given base types, i.e. only those classes will be
	 * run which extend one of the base types.
	...
	/**
	 * The <code>ExcludeBaseTypeFilter</code> annotation filters all test classes
	 * to be run by one or several given base types, i.e. only those classes will be
	 * run which <em>do not extend</em> any of the base types.
	 * Default is <code>Object.class</code>.
	 */
	@Retention(RetentionPolicy.RUNTIME)
	@Target(ElementType.TYPE)
	...
	/**
	 * The <code>ClasspathProperty</code> specifies the System property name used
	 * to retrieve the java classpath which is searched for Test classes and suites.
	...
	/**
	 * The <code>BeforeSuite</code> marks a method that will be run before the suite is run.
	 */
	@Retention(RetentionPolicy.RUNTIME)
	...
	private static ClassesFinder createFinder(Class<?> suiteClass, ClassesFinderFactory finderFactory) {
		return finderFactory.create(getSearchInJars(suiteClass), getClassnameFilters(suiteClass), getSuiteTypes(suiteClass), getBaseTypes(suiteClass), getExcludedBaseTypes(suiteClass), getClasspathProperty(suiteClass));
	}
	...
	private boolean isPublicStaticVoid(Method method) {
		return method.getReturnType() == void.class && method.getParameterTypes().length == 0 && (method.getModifiers() & Modifier.STATIC) != 0;
	}
}
